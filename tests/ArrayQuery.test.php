<?php 
class ArrayQueryTests extends \Enhance\TestFixture
{
  private $target;
	private $array;
	...
	public function setUp()
	{
		...
		include_once('src/ArrayQuery.class.php');
		include('data/data_big.php');
		$this->array=$array; //set from data file
		$this->target = \Enhance\Core::getCodeCoverageWrapper('ArrayQuery',array($this->array));
	}
	...
	/**
	 * test find in a bulk scenario
	 */
	/*
	public function testFind()
	{
		//$result = $this->target->find(array("release.year" => 2013));
		$scenario = \Enhance\Core::getScenario($this->target, 'find');
        $scenario->with(array("release.year" => 2013))
        			->expect(array(1545=>$this->array[1545],1546=>$this->array[1546]));
        $scenario->with(array("release.arch"=>"x86"))
        			->expect(array(1543=>$this->array[1543],1545=>$this->array[1545]));
        $scenario->with(array("release.arch" => array('$regex' => "/4$/")))
        			->expect(array(1544=>$this->array[1544]));
        $scenario->with(
						array(
						    "release.version" => array(
									'$mod' => array(
        ...
        $scenario->with(
        			...
        			->expect(array(1546=>$this->array[1546]));
        $scenario->with(
						array(
						    "release.arch" => array(
									'$all' => array(
